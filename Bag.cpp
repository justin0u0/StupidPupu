...
Bag::Bag() : Image("bag.png", 0, 0
	...
	for (int i = 0; i < Size; i++) {
		...
		bag_cell.emplace_back(new BagCell(i, x, y));
		bag_cell[i]->SetCallback(std::bind(Bag::MixUpdate, this, std::placeholders::_1));
	}
	// id = 20
	bag_cell.emplace_back(new BagCell(Size, screenW / 2 + 319, screenH / 2 + 5));
	...
	for (int i = 1; i < MixSize; i++) {
		...
		bag_cell.emplace_back(new BagCell(Size + i, x, y));
...
void Bag::BagUpdate() { 
	...
	for (auto m : package) {
		if (m.second.first != 0) {
			bag_cell[i]->Update(m.first->GetImage(), m.first->name, m.second.first);
			i++;
		}
	}
}
void Bag::MixUpdate(int id) {
	...
	if (id < Size) { // click bag_cell
		int ok = 0;
		for (int i = 1; i < MixSize; i++) {
			Log(Debug) << "Bag cell " << i + Size << " is_empty: " << bag_cell[i + Size]->is_empty;
		...
		if (ok) { // change the img from bag_cell[id] to mix_cell[ok]
			bag_cell[ok]->ChangeItem(*bag_cell[id]);
			auto iter = package.begin();
			for (auto iter = package.begin(); iter != package.end(); iter++) {
				std::string s = (iter->second).second;
				if (s == bag_cell[id]->name->text) {
					package.erase(iter->first);
					Log(Info) << "delete " << bag_cell[id]->name->text << " in bag";
					break;
				}
			}
			BagUpdate();
...
void Bag::AddItem(Item* item, int amount) {
	if (package.count(item))
		package.at(item).first += amount;
	else
		package.insert(make_pair(item, make_pair(amount, item->name)));
	BagUpdate();
}
